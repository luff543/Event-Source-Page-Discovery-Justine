...
class ValiReplayBuffer():
    ...
    def store(self, states_actor, states_critic, action, reward):
        self.states_actor.append(states_actor)
        self.states_critic.append(states_critic)
        self.actions.append(action)
        self.rewards.append(reward)
    ...
    def updatetobuffer(self,url,url_idx,types,env):
        for batch in range(1):
            ...
            while assent > 0 :
                ...
                s_prime, s_critic_prime,done, r = self.step(action_url, action_at, epsilon, env)
                if s_prime == None:
                    s = s
                    s_critic = s_critic
                ...
                if types == 'train':
                    ...
                    vali_memory.store(s, s_critic, action_idx, r)
                ...
                if assent <= 0 or t == 9:  # 回合結束
                    ...
                    break
                t += 1
            print('types:{}, target url:{} ,ans:{}, total_action_idx:{},total step click_url:{}'.format(types,target_url
                                                                     ,precision,total_action_idx,total_click_url))
            p_1 = precision_score(precision, [1] * len(precision))
            self.saveReward(score, cost,t+1,p_1, types)
            memory_step.store(t+1)
        # self.saveReward(score, score_2, p_1, types)
    def step(self,action_url,clickAT,epsilon,env):
        nextTrainFeatures = []
        nextCriticFeatures = []
        try:
        ...
        return nextTrainFeatures,nextCriticFeatures,ans,reward
...
def epoch_compute_loss(actor,critic,
    ...
    for i,click_step in enumerate(change_step_memory):
        ...
        try:
            ...
            #optimizer_actor.apply_gradients(zip(actor_clip_grads,actor.trainable_variables))
...
if __name__ == '__main__':
    ...
    print_interval = 5
    numofthread = 1
    savemodelname = "pretrain_A2C_tagpath_step_nofix_top3_cost0.4_fixreward3"
    savemodelname_dir = 'pretrain_A2C_tagpath_step_nofix_top3_cost0.4_fixreward3'
    ...
    for n_epi in range(51):  # 訓練次數
        ...
        if n_epi % print_interval == 0 and n_epi != 0:
            ...
            actor.save('{}/Actor_{}_epoch{}.h5'.format(savemodelname_dir, savemodelname, n_epi), save_format="tf")
            critic.save('{}/Critic_{}_epoch{}.h5'.format(savemodelname_dir, savemodelname, n_epi), save_format="tf")

            vali_p_1 = precision_score(vali_memory.rewards,[1]*len(vali_memory.rewards))
            print('vali_p_1:',vali_p_1)
            if savelastpre == None:
                savelastpre = vali_p_1
                actor.save('{}/Actor_{}_choose.h5'.format(savemodelname_dir, savemodelname), save_format="tf")
                critic.save('{}/Critic_{}_choose.h5'.format(savemodelname_dir, savemodelname), save_format="tf")
            ...
            vali_memory.clear()
            memory_step.clear()
        Tool.SaveObj(config.get('Main', 'url_feature_file_path_title'), url_featuresMap)
