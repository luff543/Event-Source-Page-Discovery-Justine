    ...
    for i in range(10):  # 訓練10次
        ...
        with tf.GradientTape() as tape:  # 在 tf GradientTape 區塊一次為一個批次進行預測
            for transition in mini_batch:
                s, a, r, s_prime, done_mask,_ = transition
            # for each_s,each_s_prime in zip(s,s_prime):
                for pre_page, page in zip(s[len(s)-1],s_prime[len(s_prime)-1]):
                    ...
                    value = q(q_features)
                    value2 = q_target(target_q_features)
                    # print('vlue',value)
        ...
        grads = tape.gradient(modelloss, q.trainable_variables)  # 使用tape 計算loss對各個可訓練變數的梯度
...
if __name__ == '__main__':
    ...
    print_interval = 5
    numofthread = 2
    savemodelname = "pretrain_dqn_without_coor_fixreward"
    savemodelname_dir = 'pretrain_dqn_without_coor_fixreward'
