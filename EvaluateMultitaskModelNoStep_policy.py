...
def add_arg():
    ...
    return modelname, dataset, numofthread
...
def Step(url, env,threshold):
    ...
    while do_step:
        ...
        try:
            ...
            break
        if (len(tmpAT) == 0) or len(tmpAT) > 500:
            break
...
class Worker(threading.Thread):
    def __init__(self, queue, env, threshold):
    ...
    def run(self):
        ...
        while self.queue.qsize() > 0:
            ...
            try:
                ...
                pass
            Step(msg, self.env,self.threshold)
        try:
            global_lock.release()
        except:
            pass
...
# %%
def UseThreadRunData(datalist, envs,threshold):
    #global workers
    workers = []
    my_queue = Queue()
    ...
    for i in range(len(envs)):
        w = Worker(my_queue,envs[i],threshold)
        workers.append(w)
    for i in range(len(envs)):
        workers[i].start()
    for i in range(len(envs)):
...
if __name__ == '__main__':
    modelname, dataset, numofthread = add_arg()
    ...
    if dataset == 'testdata' or dataset == 'both':
        ...
        UseThreadRunData(list(testwebsiteInfo.groups),envs,0)
        clickvalue = []
    ...
    if dataset == 'mixtestdata' or dataset == 'both':
        global_metric = GlobalMetric()
        UseThreadRunData(testlist,envs,0)
        clickvalue = []
        for i in global_metric.clickValue:
            tmp = []
            for j in i:
