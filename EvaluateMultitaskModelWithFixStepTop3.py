...
def get_action_index(model,state_features,actionClickHref, data, top3_data,step, depth,FeatureNotNull):
    pred_value = []
    null =[]
    url = []
    AT = []
...
def Step(url, previousTrainFeatures, depth,env):
    ...
    while True:
        ...
        if FeatureNotNull:
            actionIndex, clickhref, clickAT,predictvalue = get_action_index(agentmodel, TrainFeatures, actionClickHref,
                                                                            data, top3_data,step, depth,True)
        else:
            actionIndex, clickhref, clickAT,predictvalue = get_action_index(agentmodel, TrainFeatures,actionClickHref,
                                                                            data, top3_data, step, depth, False)
        step += 1
    ...
    def run(self):
        ...
        while self.queue.qsize() > 0:
            ...
            if prev_observation == None or url == None:
                ...
                Step(url,prev_observation, self.steps, self.env)
        try:
            global_lock.release()
        except:
            pass
