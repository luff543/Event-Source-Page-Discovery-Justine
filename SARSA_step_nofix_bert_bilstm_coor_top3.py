...
class Agent():
    ...
    def sample_action(self,state_features, data, top3_data,actionClickHref,epsilon,
        ...
        if not useprestate:
            ...
            try:
                ...
                data['step'].extend(list([1] * len(AT[0])))
            except Exception:
            ...
            try:
                ...
                top3_data['step'].extend(list([1] * len(AT[0])))
            except Exception:
        ...
        top3_data['step'].extend(list([1] * 3))

        # print('second top_3_data', len(top3_data['q(s,a)']))
        # print('data',len(data['url']))

        return new_action_index,clickhref,clickAT
    def sample_action2(self, target_url,state_features, current_url, data, top3_data,actionClickHref,epsilon,step,
        ...
        if not useprestate:
            ...
            try:
                ...
                top3_data['index'].extend(null)
                top3_data['step'].extend(null)
        # print('function 2 first top_3_data', len(top3_data['q(s,a)']))
        if step == 0:
        ...
        # print('function2 second top_3_data', len(top3_data['q(s,a)']))
        # print('top3_data',top3_data)
        # print('function2 data',len((data['url'])))
    ...
    def saveReward(self,stepreward,cost,t,p_1,types):
        ...
        if not os.path.exists(path):
            ...
            with open('{}/{}/reward/reward_{}.csv'.format('precision_result', savemodelname, types), 'w',
                ...
                writer.writerow(['step reward','cost','step','p_1'])
        with open('{}/{}/reward/reward_{}.csv'.format('precision_result', savemodelname, types), 'a',
                  newline='') as csvfile:
            if types == 'train':
                writer = csv.writer(csvfile)
                writer.writerow([stepreward,cost,t+1,p_1])
            else:
                writer = csv.writer(csvfile)
                writer.writerow([stepreward,cost,t+1,p_1])
    def updatetobuffer(self,pre_s,url,pre_coor,types,env):
        s = [pre_s]
        coor = []
        target_url = url
        ...
        score = 0.0
        precision = []
        action_idx, action_url, action_at = self.sample_action(s,data,
                                                               top3_data, alreadyclickurl, epsilon,
                                                               types, useprestate)
        current_url = action_url
        # total_click_url.append(current_url)
        t = 0
        assent = 1
        s_prime_count = 0
        while assent > 0 or s_prime_count < 5:
        # for t in range(6):  # 一個回合最大時間戳 (updating N times)
            # 根據當前Q網路提取策略，並改進策略
            if s_prime_count >= 4:
                data.clear()
                top3_data.clear()
                break
            s_prime, done, r,next_coor = self.step(current_url, s, epsilon, t,env)
            alreadyclickurl.append(current_url)
            if s_prime == None:
                s_prime = s
                current_url = current_url
                pre_coor = coor
                useprestate = True
                action_idx_, action_url_, action_at_ = self.sample_action2(target_url, s_prime, current_url, data,
                                                                           top3_data, alreadyclickurl, epsilon, t,
                                                                           types, useprestate, log_model_output)
                current_url = action_url_
                action_idx = action_idx_
                action_at = action_at_
                pre_coor = next_coor
                coor = pre_coor
                s = s_prime
                useprestate = False
            ...
            total_click_url.append(current_url)
            print('click_url:{}, reward:{}, AT:{}, next_click_url:{}, next_click_AT:{}'.format(current_url, r,
                                                                                 action_at, action_url_,action_at_))

            done_mask = 0.0 if done else 1.0  # 結束標誌遮罩(mask)

            if types == 'train':
                memory.put((s, action_idx, r, s_prime, action_idx_,done_mask, pre_coor))
            else:
                vali_memory.put((s, action_idx, r, s_prime,action_idx_,done_mask, pre_coor))
        ...
        self.saveReward(score,cost ,t,p_1, types)
    ...
    for i in range(10):  # 訓練10次
        ...
        with tf.GradientTape() as tape:  # 在 tf GradientTape 區塊一次為一個批次進行預測
            for transition in mini_batch:
                s, a, r, s_prime, a_,done_mask,coor = transition
...
if __name__ == '__main__':
    ...
    print_interval = 5
    numofthread = 2
    savemodelname = "pretrain_SARSA_step_nofix_bertbisltm_top_3_fixreward"
    savemodelname_dir = 'pretrain_SARSA_step_nofix_bertbisltm_top_3_fixreward'
    ...
    for n_epi in range(51):  # 訓練次數
        ...
        if n_epi % print_interval == 0 and n_epi != 0:
            ...
            for transition in vali_mini_batch:
                _, _, _, _,_, done_mask,_ = transition
                if done_mask == 0:
                    ans = 1
                    done_ans.append(ans)
                else:
